...
export class LessonsService {
  ...
  constructor(
    ...
    private readonly lmsElasticsearchService: LMSElasticsearchService, // Add this
  ) {}
  ...
  async create(
    ...
    try {
      ...
      const lessonData = {
        ...
        attemptsGrade:
          createLessonDto.attemptsGrade || AttemptsGradeMethod.HIGHEST,
  ...
  async findByTestId(
    ...
    try {
      ...
      // --- Elasticsearch sync for lesson test data ---
      try {
        this.logger.log(
          `Syncing lesson test data to Elasticsearch for testId: ${testId}, lessonId: ${lesson.lessonId}`,
        );

        // Get user tracking data for this lesson to find users who have attempted this test
        const lessonTracks = await this.lessonTrackRepository.find({
          ...
          select: ['userId', 'status', 'completionPercentage', 'timeSpent'],
        });

        if (lessonTracks.length > 0) {
          // Get unique user IDs who have attempted this lesson
          const userIds = [
            ...new Set(lessonTracks.map((track) => track.userId)),
          ];

          // Sync each user's data to Elasticsearch
          for (const userId of userIds) {
            try {
              // Trigger comprehensive sync via user-microservice
              const userMicroserviceUrl =
                process.env.USER_MICROSERVICE_URL || 'http://localhost:3002';

              await axios.post(
                `${userMicroserviceUrl}/user/v1/elasticsearch/users/${userId}/sync`,
                {
                  eventType: 'lesson_test_access',
                  lessonData: {
                    lessonId: lesson.lessonId,
              ...
              this.logger.log(
                `Successfully synced lesson test data for userId: ${userId}, testId: ${testId}`,
              );
            } catch (syncError) {
              this.logger.error(
                `Failed to sync lesson test data for userId: ${userId}, testId: ${testId}:`,
                syncError,
              );
          ...
          this.logger.log(
            `No lesson tracks found for lessonId: ${lesson.lessonId}, testId: ${testId}`,
          );
        }
      } catch (elasticsearchError) {
        this.logger.error(
          `Failed to sync lesson test data to Elasticsearch for testId: ${testId}:`,
          elasticsearchError,
        );
  ...
  /**
   * Find user-specific lesson by test ID (NO multiple user syncing)
   * This method returns lesson data for a specific user only, preventing
   * the multiple user syncing issue that occurs with findByTestId
   *
   * @param userId The specific user ID to get data for
   * @param testId The test ID to find the lesson for
   * @param tenantId The tenant ID for data isolation
   * @param organisationId The organization ID for data isolation
   * @returns Lesson data with user-specific course structure
   */
  async findUserLessonByTestId(
    userId: string,
    testId: string,
    tenantId: string,
    organisationId: string,
    ...
    try {
      this.logger.log(
        `Finding user-specific lesson by test ID: userId=${userId}, testId=${testId}`,
      );
      ...
      if (cachedLesson) {
        this.logger.log(
          `Returning cached user-specific lesson for userId: ${userId}`,
        );
        return cachedLesson;
      }
      ...
      // Get user-specific tracking data (NO multiple user syncing)
      const userLessonTrack = await this.lessonTrackRepository.findOne({
        where: {
          lessonId: lesson.lessonId,
          userId: userId,
          tenantId: tenantId,
        ...
        select: ['status', 'completionPercentage', 'timeSpent'],
      });

      // Build user-specific response (NO Elasticsearch sync for multiple users)
      const userSpecificLesson = {
        lessonId: lesson.lessonId,
        title: lesson.title,
        status: lesson.status,
        format: lesson.format,
        ...
        userTracking: userLessonTrack
          ? {
              status: userLessonTrack.status,
              completionPercentage: userLessonTrack.completionPercentage,
              timeSpent: userLessonTrack.timeSpent,
            }
          : null,
        testId: testId,
        // NO multiple user syncing - only return data for this specific user
      };

      // Cache the user-specific lesson data
      await this.cacheService.set(
        cacheKey,
        userSpecificLesson,
        this.cacheConfig.LESSON_TTL,
      ...
      this.logger.log(
        `Successfully found user-specific lesson for userId: ${userId}, testId: ${testId} (NO multiple user syncing)`,
      );

      return userSpecificLesson;
    } catch (error) {
      this.logger.error(
        `Error finding user-specific lesson by test ID for userId: ${userId}, testId: ${testId}: ${error.message}`,
        error.stack,
      );
      if (error instanceof NotFoundException) {
        throw error;
